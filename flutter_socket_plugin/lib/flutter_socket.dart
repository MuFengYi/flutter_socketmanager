...
class FlutterSocket {
  ...
  Future<bool?> createSocket(String host, int port, {timeout = 30}) async {
    Map arguments = {"host": host, "port": port, "timeout": timeout};
  ...
  /// @Date: 2019-08-21
  ///
  connectListener(CallBackClosure closure) async =>
      await _listen(connected, closure);
  ...
  /// @Date: 2019-08-21
  ///
  receiveListener(CallBackClosure closure) async =>
      await _listen(receive_message, closure);
  ...
  /// @Date: 2019-08-21
  ///
  disconnectListener(CallBackClosure closure) async =>
      await _listen(disconnect, closure);
}
