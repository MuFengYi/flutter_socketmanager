...
@interface PlayViewController ()<XDXVideoDecoderDelegate,XDXFFmpegVideoDecoderDelegate,XDXFFmpegAudioDecoderDelegate, XDXSortFrameHandlerDelegate>
{
...
- (void)viewDidLoad {
    ...
    [self startRenderAVByOriginWithFileName:@"rtmp://ns8.indexforce.com/home/mystream"];
#pragma mark ------下面是使用ijkplayer播放
...
- (void)startRenderAVByFFmpegWithFileName:(NSString *)fileName {
    ...
    XDXAVParseHandler *parseHandler = [[XDXAVParseHandler alloc] initWithPath:@"rtmp://ns8.indexforce.com/home/mystream"];
...
- (void)startRenderAVByOriginWithFileName:(NSString *)fileName {
    NSString *path = [[NSBundle mainBundle] pathForResource:fileName ofType:@"MOV"];
    XDXAVParseHandler *parseHandler = [[XDXAVParseHandler alloc] initWithPath:fileName];
    ...
    AudioStreamBasicDescription audioFormat = {
        .mSampleRate         = static_cast<Float64>(parseHandler.sampleRate),
        .mFormatID           = kAudioFormatMPEG4AAC,
        .mChannelsPerFrame   = static_cast<UInt32>(parseHandler.channels),
    ...
    XDXAduioDecoder *audioDecoder = [[XDXAduioDecoder alloc] initWithSourceFormat:audioFormat
                                                                     destFormatID:kAudioFormatLinearPCM
                                                                       sampleRate:static_cast<Float64>(parseHandler.sampleRate)
...
//                 Put audio data from audio file into audio data queue
                ...
                [[PCMPlayer shareInstance] playWithData:[NSData dataWithBytes:destBufferList->mBuffers->mData length:destBufferList->mBuffers->mDataByteSize]];
                
                
//                [[AAQueuePlayer shareInstance] playData:[NSData dataWithBytes:destBufferList->mBuffers->mData length:destBufferList->mBuffers->mDataByteSize]];
//                 control rate
                
                [self->audioData appendBytes:destBufferList->mBuffers->mData length:destBufferList->mBuffers->mDataByteSize];
                
                
                
                NSLog(@"audioData=========%@",self->audioData);
...
//            [[AAQueuePlayer shareInstance] playData:[NSData dataWithBytes:audioInfo->data length:audioInfo->dataSize]];
        }
    }];
}
...
- (IBAction)saveAacData:(id)sender {
    ...
    [self->audioData writeToFile:filePath atomically:YES];
}
