...
- (VTDecompressionSessionRef)createDecoderWithVideoInfo:(AAVideoInfo *)videoInfo videoDescRef:(CMVideoFormatDescriptionRef *)videoDescRef videoFormat:(OSType)videoFormat lock:(pthread_mutex_t)lock callback:(VTDecompressionOutputCallback)callback decoderInfo:(XDXDecoderInfo)decoderInfo {
    pthread_mutex_lock(&lock);
    
    OSStatus status;
    if (videoInfo->videoFormat == AAH264VideoFormat) {
        const uint8_t *const parameterSetPointers[2] = {decoderInfo.sps, decoderInfo.f_pps};
        const size_t parameterSetSizes[2] = {decoderInfo.sps_size, decoderInfo.f_pps_size};
        status = CMVideoFormatDescriptionCreateFromH264ParameterSets(kCFAllocatorDefault,
                                                                     ...
                                                                     4,
                                                                     videoDescRef);
    }else if (videoInfo->videoFormat == AAH265VideoFormat) {
        if (decoderInfo.r_pps_size == 0) {
            const uint8_t *const parameterSetPointers[3] = {decoderInfo.vps, decoderInfo.sps, decoderInfo.f_pps};
            const size_t parameterSetSizes[3] = {decoderInfo.vps_size, decoderInfo.sps_size, decoderInfo.f_pps_size};
            ...
            const size_t parameterSetSizes[4] = {decoderInfo.vps_size, decoderInfo.sps_size, decoderInfo.f_pps_size, decoderInfo.r_pps_size};
            if (@available(iOS 11.0, *)) {
                status = CMVideoFormatDescriptionCreateFromHEVCParameterSets(kCFAllocatorDefault,
                                                                             4,
                                                                             parameterSetPointers,
...
- (void)getNALUInfoWithVideoFormat:(AAVideoFormat)videoFormat extraData:(uint8_t *)extraData extraDataSize:(int)extraDataSize decoderInfo:(XDXDecoderInfo *)decoderInfo {
    ...
    for (int i = 0; i < size; i ++) {
        if (i >= 3) {
            if (data[i] == 0x01 && data[i - 1] == 0x00 && data[i - 2] == 0x00 && data[i - 3] == 0x00) {
                if (videoFormat == AAH264VideoFormat) {
                ...
                }else if (videoFormat == AAH265VideoFormat) {
...
#pragma mark Decode
- (void)startDecode:(AAVideoInfo *)videoInfo session:(VTDecompressionSessionRef)session lock:(pthread_mutex_t)lock {
    pthread_mutex_lock(&lock);
    ...
    XDXDecodeVideoInfo *sourceRef = (XDXDecodeVideoInfo *)malloc(sizeof(videoInfo));
